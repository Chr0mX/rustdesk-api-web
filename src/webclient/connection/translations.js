// bridge.dart's translate() (flutter/lib/web/bridge.dart) always calls out
// to getByName("translate", {locale, text}) - there's no local Dart-side
// dictionary fallback for the web platform at all (unlike native builds,
// which generate their own translation maps at build time). Most call
// sites pass the literal English text as the lookup key (a short button
// label like "Settings"), which is why most of the UI reads fine even with
// no dictionary here at all - the "translation" is a no-op. But some call
// sites (mostly longer tooltip/help text) pass a snake_case/kebab-case
// *key* instead, expecting a real lookup - unhandled, that key shows up
// verbatim in the UI instead of the sentence it names (confirmed live:
// "empty_recent_tip", "show_monitors_tip",
// "allow-remote-toolbar-docking-any-edge", etc. rendering as-is on the
// Recents tab and in Settings).
//
// Hand-copied from the recovered engine's own real English source -
// Chr0mX/rustdesk's src/lang/en.rs, the canonical (KEY, English text)
// table every other language's translation file is keyed against - not
// guessed or reworded. Deliberately not exhaustive (see docs/
// WEBCLIENT_V2_REBUILD_PLAN.md's connection/README.md item 4: the
// gen_js_from_hbb.py generator v1 used to produce something like this
// automatically was deliberately not carried over) - covers what that
// file has as of this snapshot; a key genuinely missing from here still
// falls back to rendering as-is, same as before, just for a smaller set
// of keys than today.
export default {
