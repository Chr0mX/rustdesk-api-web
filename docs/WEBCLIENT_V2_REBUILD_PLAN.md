...
39 confirmed working, 5 wired to an explicit stub (visible in the UI,
does nothing), 9 have no handler at all (silently unhandled), 12 are a
...
upload, cancel), and terminal support end to end (PR #58 - open/close,
input, resize, output) close out most of the rest; see below for what's
still open. A published audit artifact with the full per-feature
breakdown exists alongside this doc (not repo-tracked, ask if you need
the link regenerated).

**Working** (includes many fixed live this session, via PRs #43-#58):
...
`FileResponse.block`/`.done` streaming), and cancel transfer job
(`FileAction.cancel`). Field names for all of the file-transfer work were
confirmed against both the real `message.proto` (fetched directly) and
the already-committed legacy v1 `message.ts`
...
truth without a live connection), and terminal open/close/input/resize/
output (`TerminalAction.open`/`.close`/`.data`/`.resize`, results routed
to `pushEvent("terminal_response", ...)` - field names confirmed against
`message.proto`'s own "standalone feature like FileAction" comment and
against `TerminalModel.handleTerminalResponse`, which already anticipates
a web build in its own comments).
...
`input_key`'s own `mapKey()`/`KEY_MAP`, see item 4 above), file transfer's
overwrite confirmation/resume/compression (`confirm_override_file` ties
into the same digest/skip state machine as `remove_all_empty_dirs` above -
both plausible `FileAction` mappings for the recursive-delete ops fit the
...
`last_audit_note`, `platform`, `resolve_avatar_url`, `local_os`).
...
Recommended order (impact vs. effort), updated: keyboard Map Mode → file
transfer's recursive-delete/overwrite-confirm/resume/compression/
directory-transfer remainder (needs a live connection to disambiguate,
see above) → remaining small getters.
