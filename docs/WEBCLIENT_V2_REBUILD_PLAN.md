...
46 confirmed working, 3 wired to an explicit stub (visible in the UI,
...
#54), local→remote clipboard (already worked, re-audited - no code
...
platform, message boxes, envvar, misc info), chat and keyboard Map Mode
(PR #63), and (PR #65) live online-status polling plus a real fix for a
broken audio startup-race "fix" close out most of the rest.
...
`.resize`, results routed to `pushEvent("terminal_response", ...)`), the
remaining small getters - peer platform (sourced from the already-stored
`PeerInfo`), peer-sent `message_box` (routed to `globals.msgbox()`),
`envvar` (a plain per-browser localStorage KV store per `bridge.dart`'s
own comment), `build_date`/`conn_session_id`/`last_audit_note` (sensible
defaults), and (PR #69) `my_name` (`getByName("my_name")` was previously
unhandled - confirmed against the real legacy bundle's own
`case"my_name": return Ga()`, which is a hardcoded literal `"web"`, not
derived from anything browser-specific; matched exactly rather than
inventing a more descriptive value). `resolve_avatar_url`/`local_os`
turned out not to need fixing at all on inspection - the former's Dart
caller already falls back to the input avatar string when unhandled, and
the latter isn't called from `bridge.dart` anywhere. Field names for the
file-transfer/terminal/keyboard work were confirmed against real
...
**Correction (retracts most of PR #65/#67's online-status claim) -
`query_onlines` is NOT working, and can't be made to work as designed.**
An earlier pass here concluded live online-status polling needed new
backend work; a later pass "corrected" that to "hbbs already supports
this natively via `RendezvousMessage.online_request`/`.online_response`
over the same WebSocket `_start()`'s `punch_hole_request` already talks
to" and shipped `queryOnlines()` in `curConn.js` on that basis - ported
faithfully from the real, up-to-date legacy webclient bundle's own
`x1()`/`ce()` functions, not guessed. Both the legacy bundle and this
port send `OnlineRequest` to the exact same WebSocket endpoint
`_start()` uses (`ce()`/`getDefaultUri()`, port `+2` from the base
rendezvous port). The second pass was still wrong, just less obviously -
read against `rustdesk/rustdesk-server`'s actual `rendezvous_server.rs`
(checked across `master` and the last four release tags, consistently):
that WebSocket-upgradeable port's message dispatcher, `handle_tcp`
(reached via `handle_listener_inner`'s `ws: true` branch), has NO match
arm for `Union::OnlineRequest` at all - only `handle_listener2`, a
*separate, plain-TCP-only* listener bound to a *different* port
(`nat_port`, the base rendezvous port `- 1`, no WebSocket upgrade
support in its code path whatsoever), handles it
(`handle_online_request`). `handle_tcp`'s catch-all arm for an
unrecognized message returns `false`; `handle_listener_inner`'s read
loop treats that as "stop", dropping the connection immediately -
exactly matching the "`WebSock.onopen` immediately followed by
`WebSock.onclose` with code 1006" pattern seen live, repeatedly, right
after each address book's peer list loads (once per address book -
`queryOnlines()` is called per-list, which is what the "storm" from an
earlier round of this investigation actually was; that round's "probably
just page-reload noise" guess was wrong too).

Since browsers cannot open raw TCP sockets, `OnlineRequest` is
structurally unreachable from any browser-based client - not a bug in
this port specifically, a real gap in what a browser-based RustDesk
client can ever do against a stock, unmodified `hbbs`. The legacy
bundle's identical implementation would fail exactly the same way
against the exact same server; whether it's ever appeared to work is
more likely explained by a different `rustdesk-server` fork/build, a
reverse-proxy quirk, or simply never having been exercised against a
live server before, than by any actual difference in behavior. Only a
real fix exists from here: rustdesk-api's own Go backend (not
browser-limited) opening the raw TCP connection to `nat_port` itself and
exposing the result over `/api`, or accepting this as a platform
ceiling like LAN discovery. Not attempted in this pass - flagging for a
decision rather than guessing further. `queryOnlines()`'s code is left
in place (harmless - it fails closed, `callback_query_onlines` reports
peers offline rather than crashing) but should not be trusted as "done."

Also fixed live: an earlier libopus.js startup-race "fix" (meant to stop
`Decoder_new called before runtime initialization` crashes) was itself
broken - it recreated the worker and immediately resent the same
message on every crash, which just re-hit the identical race almost
every time, spinning in a visible crash loop that re-fetched
libopus.js/libopus.wasm repeatedly and never actually produced sound
(confirmed via two side-by-side browser console logs, webclient vs.
legacy). Real fix: a small Blob-URL wrapper predefines
`LIBOPUS_WASM_URL`/`Module.onload` (hooks libopus.js's own header
comment already invites reuse of - "Sometimes an existing Module object
exists with properties meant to overwrite the default module
functionality") before `importScripts`-ing the real file, and only sends
real init/decode messages once that `onload` hook actually fires -
genuinely gated on readiness instead of a timing guess.

**Stubbed** (UI present, does nothing): per-session login 2FA
(`send_2fa` - real, reachable code, not desktop-only; distinct from
account-level 2FA setup below, which *is* platform ceiling), audit notes
(`send_note`/`setAuditGuid` - the audit-server URL itself is wired), and
file transfer's recursive-delete flow (`remove_all_empty_dirs`/
`read_dir_to_remove_recursive` - deliberately left alone, see "Missing"
below).
