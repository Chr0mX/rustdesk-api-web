...
**Correction to the correction: `query_onlines` IS working - confirmed
live by the user, real green dots on real online machines - but only
because of which `hbbs` binary is actually deployed, which isn't
documented anywhere in this repo.** A previous pass here retracted the
"working" verdict after finding that `rustdesk/rustdesk-server` (the
*upstream/vanilla* repo)'s WebSocket-reachable message dispatcher
(`handle_tcp`, reached via `handle_listener_inner`'s `ws: true` branch -
the same port `_start()`'s `punch_hole_request` uses) has no match arm
for `Union::OnlineRequest` at all, checked across `master` and the last
four release tags. That reading of the vanilla source was accurate, and
the mechanical failure mode it predicted (unmatched message → `handle_tcp`
returns `false` → connection drops → matches the "`WebSock.onopen`
instantly followed by `onclose` code 1006" pattern seen live) is real -
but it doesn't describe what's actually deployed here. **This project's
`Rustdesk-Server-Installer` (`Chr0mX/Rustdesk-Server-Installer`, see its
`lib.sh` - `HBBS_OWNER`/`HBBS_REPO` default to `Chr0mX`/`rustdesk-server`)
builds hbbs/hbbr from `Chr0mX/rustdesk-server`, a fork-of-a-fork
(`rustdesk/rustdesk-server` → `lejianwen/rustdesk-server` →
`Chr0mX/rustdesk-server`) that adds exactly this: an `OnlineRequest` arm
inside `handle_tcp` itself** (traced to upstream-of-fork commits
`e5ac30e "feat: Add online request from webclient"` and `872f1f9 "feat:
优化并加入websocket支持"` - "optimize and add WebSocket support" - both by
`lejianwen`), answering the query directly on the same WS connection
`_start()` already uses, in addition to (not instead of) the vanilla
`handle_listener2`/`nat_port` raw-TCP path. Same wire format
(`RendezvousMessage.online_request`/`.online_response`), so
`queryOnlines()` in `curConn.js` needed no code change once this was
found - it was already correct against the actual deployed server, just
verified against the wrong hbbs source the first time.

**Load-bearing consequence for anyone maintaining or redeploying this
webclient**: it is coupled to `Chr0mX/rustdesk-server`, not vanilla
`rustdesk/rustdesk-server` - deploying this webclient against a stock
hbbs (or a `Rustdesk-Server-Installer` run with `HBBS_OWNER`/`HBBS_REPO`
overridden away from the default) will reproduce the exact 1006-storm
failure mode described above, since the vanilla server genuinely lacks
this handler. If online-status polling ever regresses, check which hbbs
fork is actually running before re-investigating the webclient's own
code.
