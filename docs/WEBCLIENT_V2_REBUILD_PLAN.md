...
defaults) - and live online-status polling (`query_onlines`). That last
one needed a real correction: an earlier pass here concluded it needed
new backend work, since no REST endpoint anywhere provides a live
"is this ID online" check. Wrong - decompiling the actual (up-to-date;
the local `rustdesk-api` clone used for the first pass was stale)
legacy webclient bundle found that hbbs already supports this natively
via `RendezvousMessage.online_request`/`.online_response`, the same
rendezvous server `_start()`'s `punch_hole_request` already talks to -
a short-lived WebSocket opened just for the query. Implemented for
real: `queryOnlines()` in `curConn.js` sends the request and unpacks
`OnlineResponse.states` (a bitset, MSB-first per byte). `resolve_avatar_url`/
`local_os` turned out not to need fixing at all on inspection - the
former's Dart caller already falls back to the input avatar string when
unhandled, and the latter isn't called from `bridge.dart` anywhere.
Field names for the file-transfer/terminal/keyboard/online-status work
were confirmed against real upstream sources fetched directly rather
than guessed: `message.proto`, `rendezvous.proto`, the already-committed
legacy v1 `message.ts` (generated by the same `protoc-gen-ts_proto` tool
this v2 client's own gitignored `message.ts` uses), and (for Map Mode)
the `rdev` crate's actual keycode tables.
