...
37 confirmed working, 5 wired to an explicit stub (visible in the UI,
does nothing), 11 have no handler at all (silently unhandled), 12 are a
...
change needed), and most of file transfer (PR #56 - remote dir browsing,
create/rename/remove, the local file picker, single-file download and
upload, cancel) close out most of the rest; see below for what's still
...
**Working** (includes many fixed live this session, via PRs #43-#56):
...
(open/close/resize/send-input - the whole feature area), file transfer's
...
Recommended order (impact vs. effort), updated: terminal → keyboard Map
Mode → file transfer's recursive-delete/overwrite-confirm/resume/
compression/directory-transfer remainder (needs a live connection to
disambiguate, see above) → remaining small getters.
